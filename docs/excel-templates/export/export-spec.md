...
**工作表**: Sheet1，数据从第3行开始（第1行为英文字段名，第2行为中文字段名）
**参考文件**: `dataExportReport.xlsx`（已提供，11KB）

该模板包含员工基础信息字段，采用双层表头格式（第1行英文字段名，第2行中文字段名），用于导出员工基础数据。

### payrollCalculationResult.xlsx
**用途**: 薪资计算结果导出模板，用于导出完整的薪资计算结果
**工作表**: 数据写入到`payrollCalculationResult`工作表中
**参考文件**: `payrollCalculationResult.xlsx`（已提供，175KB）
...
### 2. 薪资计算结果表 (payrollCalculationResult.xlsx)
...
| 列序号 | 列名称 | 数据类型 | 说明 | 示例 |
|--------|--------|----------|------|------|
| A | 员工编码 | 字符串 | 员工唯一标识 | RYJM-0000137269 |
...
### 2. 薪资明细表 (payrollCalculationResult.xlsx)
