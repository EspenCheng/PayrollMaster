...
**工作表**: 默认工作表名为"Sheet1"，数据从第3行开始（第1行为英文字段名，第2行为中文字段名）
...
- **英文字段名**: 第1行为英文字段名称（如 mdnb_employee.number）
- **中文字段名**: 第2行为中文字段名称（如 员工编码）
- **数据行**: 从第3行开始为实际数据
- **空行处理**: 系统会自动跳过空行
...
|------|----------|-------------|----------|----------|------|
| 1 | 员工编码 | mdnb_employee.number | 字符串 | 非空且唯一 | RYJM-0000137269 |
...
### 业务验证
1. **工龄字段**: 入井年限不能大于集团工龄
2. **考勤工数**: 各类假期工数之和不能超过制度工日工数
3. **月度总工数**: 制度工日工数、公休日出勤工数、节假日出勤工数、假期工数之和不能超过当月最大天数（28/29/30/31天）
4. **系数范围**: 岗位系数必须在1.0-6.0之间
5. **津贴标准**: 入井津贴标准只能是0、45、60

### 错误处理
- **必填字段缺失**: 系统会提示"缺少必填字段: [字段名]"
- **数据类型错误**: 系统会提示"字段 [字段名] 数据类型不正确，期望: [类型]"
- **重复数据**: 系统会提示"员工编码 [编码] 重复"
- **考勤工数超限**: 系统会提示"[字段名] 验证失败: 假期工数之和不能超过制度工日工数"
- **月度总工数超限**: 系统会提示"工数验证失败: 月度总工数不能超过当月最大天数"
...
参考 `dataSubmissionReport.xlsx` 文件，该文件包含了完整的字段示例和样例数据。
