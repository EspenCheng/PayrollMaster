...
class User(SQLModel, table=True):
    ...
    role: str = Field(index=True, description="角色：admin/staff-transfer-admin/attendance-admin/social-security-admin/finance-admin/payroll-calculator/employee")
...
3. **权限控制**：
   - 只有admin和staff-transfer-admin角色可以创建/删除用户
   - 只有admin可以访问薪资规则配置
   - 单位薪资核算员只能查看和操作本单位数据
   - 各类管理员只能访问自己职责范围内的功能模块
   - 登录时使用手机号码进行身份验证
   - **详细权限说明**: 请参考 [角色权限管理文档](./roles-permissions.md)
