...
```
excel-templates/
├── 13-Excel模板说明.md                    # 本文档，说明模板管理方式
├── import/                      # 数据导入模板目录
│   ├── dataSubmissionReport.xlsx      # 数据提报模板（已提供）
│   └── import-02-功能规格说明书.md          # 导入数据格式说明文档
├── export/                      # 数据导出模板目录
│   ├── dataExportReport.xlsx          # 数据导出模板（已提供）
│   ├── payrollCalculationResultExportReport.xlsx  # 薪资计算结果导出模板（已提供）
│   └── export-02-功能规格说明书.md          # 导出数据格式说明文档
└── web-display/                 # Web展示结构定义目录
    ├── 16-数据表结构.md  # Web数据表格展示结构说明
    └── 17-字段映射.md        # 字段映射关系文档
```
...
- **import-02-功能规格说明书.md**: 详细说明导入数据的字段格式、必填项、验证规则等

### 导出模板 (export/)
- **dataExportReport.xlsx**: 数据导出模板，用于导出员工基础信息（已提供）
- **payrollCalculationResultExportReport.xlsx**: 薪资计算结果导出模板，用于导出完整的薪资计算结果（已提供）
- **export-02-功能规格说明书.md**: 详细说明导出数据的字段结构、计算公式、格式要求等

### Web展示结构 (web-display/)
- **16-数据表结构.md**: Web界面中数据表格的展示结构和字段定义
- **17-字段映射.md**: Excel字段与Web界面字段的映射关系
...
2. **导出模板**:
   ...
   - **payrollCalculationResultExportReport.xlsx**: 薪资计算结果导出模板，用于导出完整的薪资计算结果
...
### payrollCalculationResultExportReport.xlsx (薪资结果导出模板)
- **位置**: `export/payrollCalculationResultExportReport.xlsx`
- **用途**: 用于导出完整的薪资计算结果
...
- 所有Excel模板文件必须遵循`44-薪资变量定义.md`中定义的字段映射关系
- 所有计算逻辑必须遵循`43-薪资计算逻辑.md`中定义的计算公式
