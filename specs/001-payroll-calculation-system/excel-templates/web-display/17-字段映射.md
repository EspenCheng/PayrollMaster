...
### 导出模板: payrollCalculationResultExportReport.xlsx
**路径**: `excel-templates/export/payrollCalculationResultExportReport.xlsx`
**用途**: 薪资计算结果导出模板，用于导出完整的薪资计算结果
**工作表**: payrollCalculationResult
**说明**: 该模板包含了完整的薪资计算结果字段，支持导出所有员工的薪资明细和汇总信息。

### 字段映射关系
本映射文档详细说明了dataSubmissionReport.xlsx（导入）、dataExportReport.xlsx（导出）和payrollCalculationResultExportReport.xlsx（导出）中的字段与Web界面展示字段之间的映射关系。
...
|------------|-----------|-------------|----------|----------|------|
| mdnb_employee.number | employeeCode | 员工编码 | 直接映射 | - | 唯一标识 |
| mdnb_employee.name | employeeName | 员工姓名 | 直接映射 | - | - |
| mdnb_department.number | departmentCode | 部门编码 | 直接映射 | - | - |
...
| 字段名 | 验证规则 | 错误提示 |
|--------|----------|----------|
| employeeCode | 非空且唯一 | 员工编码不能为空且不能重复 |
