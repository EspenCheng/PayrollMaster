...
3. **Given** 管理员配置了奖金计算规则（如按岗位工资的10%），**When** 系统执行薪资计算时，**Then** 自动计算并添加奖金到员工薪资中
...
1. **Given** 薪资计算已完成，**When** 管理员查看计算结果页面，**Then** 显示每个员工的详细薪资明细（岗位工资、奖金、扣除、实发等）
...
3. **Given** 管理员需要导出薪资计算结果报表，**When** 点击导出按钮，**Then** 系统生成基于 `payrollCalculationResult.xlsx` 模板的文件，包含 payrollCalculationResult 工作表
...
**Why this priority**: 薪资数据涉及敏感信息，需要严格的访问控制。系统定义了6种不同角色，每种角色都有特定的功能范围和职责分工。
...
2. **Given** 职工调配管理员账号，**When** 尝试访问薪资计算配置页面，**Then** 系统拒绝访问并提示权限不足
3. **Given** 单位薪资核算员账号，**When** 尝试查看其他单位的薪资数据，**Then** 系统拒绝访问，仅允许查看本单位数据
4. **Given** 用户输入正确登录凭证，**Then** 系统验证身份并建立安全会话
5. **Given** 用户会话超时，**Then** 系统自动注销并要求重新登录
6. **Given** 社保管理员账号，**When** 尝试访问个人所得税配置，**Then** 系统拒绝访问并提示权限不足

**角色说明**:
- **系统管理员**: 拥有所有权限
- **职工调配管理员**: 负责用户管理和职工调配业务
- **职工考勤管理员**: 负责考勤数据管理
- **社保管理员**: 负责社保相关业务
- **财务管理员**: 负责个人所得税计算
- **单位薪资核算员**: 负责本单位薪资计算和数据管理
...
- **FR-003**: 系统必须提供薪资计算规则配置界面，支持多种计算规则（固定金额、百分比、条件判断），包括奖金、扣款、加班费等参数设置
...
- **FR-008**: 系统必须支持将薪资计算结果导出为Excel文件，包含多种报表格式
  - 基于用户提供的参考文件设计：
    ...
    - `payrollCalculationResult.xlsx`：薪资计算结果导出表（payrollCalculationResult工作表）
...
- **员工数据**: 包含员工ID、姓名、部门、职位、考勤记录等基础信息
...
- **薪资核算结果导出参考**: `payrollCalculationResult.xlsx` - 用于导出薪资计算结果（包含 payrollCalculationResult 工作表）
...
- **A**: Excel表格模板文件统一管理：
  ...
  - **参考文件**: 用户提供的原始表格文件位于 `C:\Users\Espen\Desktop\` 目录下：
    ...
    - `payrollCalculationResult.xlsx` - 薪资核算结果导出参考
...
- **A**: 用户提供了三个Excel参考文件：
  ...
  - 薪资核算结果导出参考：`payrollCalculationResult.xlsx`（位于 `C:\Users\Espen\Desktop\`，包含 payrollCalculationResult 工作表）
...
- **A**: 根据用户要求，已删除税后项目区中的"保教费"和"取暖补贴"项目。所有相关文档已更新，包括：
  - 薪资数据导出格式说明文档 (`excel-templates/export/export-spec.md`)
  - 薪资计算逻辑文档 (`rules/payroll-calculation.md`)
  - 薪资变量定义文档 (`rules/payroll-variables.md`)
  当前税后项目仅包含：独生子女费、女工卫生费

- **Q**: 是否需要税后补发和税后补扣项目？
- **A**: 根据用户要求，已删除税后项目区中的"税后补发"和"税后补扣"项目。所有相关文档已更新，包括：
  - 薪资计算逻辑文档 (`rules/payroll-calculation.md`) - 从实领工资公式中删除
  - 薪资数据导出格式说明文档 (`excel-templates/export/export-spec.md`) - 删除相关列
  - 薪资变量定义文档 (`rules/payroll-variables.md`) - 删除变量定义
  - Web字段映射文档 (`excel-templates/web-display/field-mapping.md`) - 更新计算公式
  当前税后项目仅包含：独生子女费、女工卫生费
...
- **A**: 根据用户要求，已修正实领工资计算公式，添加个人所得税计算：
  ...
  - 所有相关文档已更新，包括：
    - 薪资计算逻辑文档 (`rules/payroll-calculation.md`)
    - 薪资数据导出格式说明文档 (`excel-templates/export/export-spec.md`) - 添加个人所得税列，修正计算公式
    - 规格文档 (本文档) - 更新个人所得税相关说明

### Assumptions

- 初始版本专注于基本的薪资计算功能（岗位工资、奖金、扣款、加班费）
